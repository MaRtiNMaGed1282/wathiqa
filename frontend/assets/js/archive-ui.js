...
(function (global) {
  ...
  function toast(message, success = true) {
    if (typeof global.Toastify === "function") global.Toastify({ text: message, duration: 3000, gravity: "top", position: "left", close: true, stopOnFocus: true, style: { background: success ? "#16a34a" : "#dc2626" } }).showToast();
  ...
  async function confirmArchive(label) {
    ...
    if (global.Swal?.fire) {
      const result = await global.Swal.fire({ title: "تأكيد الأرشفة", text, input: "textarea", inputLabel: "سبب الأرشفة (اختياري)", inputPlaceholder: "اكتب سبب الأرشفة...", showCancelButton: true, confirmButtonText: "أرشفة", cancelButtonText: "إلغاء", confirmButtonColor: "#b45309", reverseButtons: true });
  ...
  async function archiveRecord(type, id, label) {
    ...
    try {
      const result = await global.api.post("/archive", { entityType: type, recordId: id, reason: reason || null });
      toast(result?.message || "تمت الأرشفة بنجاح", true);
      setTimeout(() => { global.location.href = LIST_ROUTES[type]; }, 450);
  ...
  function addProfileArchiveButton() {
    ...
    button.id = "archive-record-button"; button.type = "button";
    ...
    if (header && header.classList.contains("flex")) header.appendChild(button);
    else { button.style.position = "fixed"; button.style.top = "18px"; button.style.left = "18px"; button.style.zIndex = "9999"; document.body.appendChild(button); }
  }

  function createArchiveModal() {
    if (state.modal) return state.modal;
    const overlay = document.createElement("div");
    overlay.id = "wathiqa-archive-modal";
    ...
    overlay.addEventListener("click", (event) => {
      if (event.target === overlay || event.target.closest("[data-archive-close]")) return closeArchiveModal();
      const filter = event.target.closest("[data-archive-filter]");
      if (filter) { state.activeType = filter.dataset.archiveFilter || ""; updateArchiveFilterStyles(); loadArchiveRecords(); return; }
      const restore = event.target.closest("[data-archive-restore]");
      if (restore) { restoreRecord(restore.dataset.archiveRestore); return; }
      if (event.target.closest("[data-archive-refresh]")) loadArchiveRecords();
    });
    state.modal = overlay; return overlay;
  }

  function updateArchiveFilterStyles() {
    state.modal?.querySelectorAll("[data-archive-filter]").forEach((button) => {
      const active = button.dataset.archiveFilter === state.activeType;
      button.className = active ? "archive-filter bg-primary text-white px-4 py-2 rounded-lg" : "archive-filter bg-white border border-primary text-primary px-4 py-2 rounded-lg";
    });
  }

  async function loadArchiveRecords() {
    const body = state.modal?.querySelector("[data-archive-body]"); if (!body) return;
    body.innerHTML = '<div class="py-12 text-center text-gray-500">جاري تحميل الأرشيف...</div>';
    try {
      const url = state.activeType ? `/archive?type=${encodeURIComponent(state.activeType)}` : "/archive";
      const rows = await global.api.get(url);
      if (!rows?.length) { body.innerHTML = '<div class="py-12 text-center text-gray-500">لا توجد سجلات مؤرشفة</div>'; return; }
      body.innerHTML = `<div class="overflow-x-auto"><table class="w-full text-right"><thead class="bg-gray-50"><tr><th class="p-3">النوع</th><th class="p-3">رقم السجل</th><th class="p-3">السبب</th><th class="p-3">بواسطة</th><th class="p-3">التاريخ</th><th class="p-3">الإجراء</th></tr></thead><tbody>${rows.map((row) => `<tr class="border-b"><td class="p-3">${ENTITY_LABELS[row.entity_type] || row.entity_type}</td><td class="p-3">${row.record_id}</td><td class="p-3">${row.reason || "—"}</td><td class="p-3">${row.archived_by_name || "—"}</td><td class="p-3">${row.archived_at ? new Date(row.archived_at).toLocaleString("ar-EG") : "—"}</td><td class="p-3"><button type="button" data-archive-restore="${row.id}" class="bg-green-600 hover:bg-green-700 text-white px-3 py-2 rounded-lg">استعادة</button></td></tr>`).join("")}</tbody></table></div>`;
  ...
  async function restoreRecord(id) {
    let confirmed = true;
    if (global.Swal?.fire) { const result = await global.Swal.fire({ title: "تأكيد الاستعادة", text: "سيعود السجل إلى السجلات النشطة.", icon: "question", showCancelButton: true, confirmButtonText: "استعادة", cancelButtonText: "إلغاء", reverseButtons: true }); confirmed = result.isConfirmed; }
    else confirmed = global.confirm("سيعود السجل إلى السجلات النشطة.");
    if (!confirmed) return;
    try { const result = await global.api.put(`/archive/${id}/restore`, {}); toast(result?.message || "تمت الاستعادة بنجاح", true); await loadArchiveRecords(); }
    catch (error) { console.error(error); toast(error?.message || "فشل استعادة السجل", false); }
  }

  function openArchiveModal() {
    if (!isAdmin()) return toast("ليس لديك صلاحية للوصول إلى الأرشيف", false);
    const modal = createArchiveModal(); modal.classList.remove("hidden"); modal.classList.add("flex"); state.activeType = ""; updateArchiveFilterStyles(); loadArchiveRecords();
  }
  ...
  function addArchiveLinkToSidebar() {
    if (!isAdmin()) return;
    const sidebar = document.getElementById("sidebar"); if (!sidebar || sidebar.querySelector('[data-page="archive"]')) return;
    const nav = sidebar.querySelector("nav") || sidebar.querySelector(".sidebar-nav"); if (!nav) return;
    const link = document.createElement("a"); link.href = "#archive"; link.dataset.page = "archive"; link.className = "sidebar-link flex items-center gap-3 px-4 py-3 rounded-lg transition"; link.innerHTML = '<i data-lucide="archive"></i><span>الأرشيف</span>';
    link.addEventListener("click", (event) => { event.preventDefault(); openArchiveModal(); }); nav.appendChild(link); if (global.lucide?.createIcons) global.lucide.createIcons();
  }
