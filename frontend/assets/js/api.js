(function (global) {
  ...
  function ensureLoadingBar() {
    ...
    loadingBar.style.cssText = [
      ...
      "z-index:99999","background:linear-gradient(90deg,transparent,#0f766e,transparent)",
      "background-size:200% 100%","animation:wathiqaApiLoading 1s linear infinite",
      "display:none","pointer-events:none",
    ].join(";");
    if (!global.document.getElementById("wathiqa-api-loading-style")) {
      const style = global.document.createElement("style");
      style.id = "wathiqa-api-loading-style";
      style.textContent = "@keyframes wathiqaApiLoading{0%{background-position:200% 0}100%{background-position:-200% 0}}";
  ...
  function unmarkElementBusy(element) {
    ...
    if (count <= 1) {
      ...
      if (element.dataset.wathiqaOriginalDisabled !== "true") element.disabled = false;
  ...
  function findBusyElement() {
    ...
    if (active instanceof global.HTMLInputElement && active.type === "submit") return active;
    return null;
  }
  ...
  function getBusyElement(options) {
    if (options?.busyElement instanceof global.HTMLElement) return options.busyElement;
  ...
  function isLiveServerEnvironment() {
    try {
      ...
      return (location.hostname === "localhost" || location.hostname === "127.0.0.1") && location.port === "5500";
    } catch { return false; }
  }

  function getConfiguredBaseUrl() {
    try {
      const configuredBaseUrl = global.__APP_CONFIG__?.BASE_URL ?? DEFAULT_BASE_URL;
      if (configuredBaseUrl === DEFAULT_BASE_URL && isLiveServerEnvironment()) return "http://localhost:5000/api";
  ...
  function isFormData(value) { return value instanceof FormData; }

  function isJsonSerializable(value) {
    return value !== undefined && value !== null && !isFormData(value) &&
  ...
  async function parseResponse(response, responseType) {
    if (response.status === HTTP_STATUS.NO_CONTENT || response.status === HTTP_STATUS.RESET_CONTENT) return null;
  ...
  function buildHeaders(options = {}, body) {
    ...
    if (isJsonSerializable(body) && !headers.has("Content-Type")) headers.set("Content-Type", "application/json");
    if (isFormData(body) && !headers.has("Content-Type")) headers.delete("Content-Type");
    return headers;
  }

  class ApiError extends Error {
    constructor(message, status, body = null, url = "") {
      super(message); this.name = "ApiError"; this.status = status; this.body = body; this.url = url;
      if (Error.captureStackTrace) Error.captureStackTrace(this, ApiError);
    }
  }

  function createError(message, status, body, url) { return new ApiError(message, status, body, url); }

  async function request(method, url, options = {}) {
    const baseUrl = options.baseUrl ?? getConfiguredBaseUrl();
    const requestUrl = appendQueryParams(resolveUrl(url, baseUrl), options.params);
    ...
    const mutationKey = isMutation ? getMutationKey(method, requestUrl, body) : null;
    ...
    try {
      response = await fetch(requestUrl, { ...options, method, headers, body: requestBody, signal: controller.signal });
    } catch (error) {
      clearTimeout(timeout);
      const apiError = error?.name === "AbortError"
        ? createError("انتهت مهلة الاتصال بالخادم", HTTP_STATUS.REQUEST_TIMEOUT, null, requestUrl)
        : error instanceof TypeError
          ? createError("تعذر الاتصال بالخادم", HTTP_STATUS.NETWORK_ERROR, null, requestUrl)
          : error;
      if (apiError instanceof ApiError) {
        global.__WATHIQA_LAST_API_ERROR__ = { method, url: requestUrl, message: apiError.message, status: apiError.status };
    ...
    if (response.status === HTTP_STATUS.UNAUTHORIZED) {
      ...
      const error = createError("غير مصرح", HTTP_STATUS.UNAUTHORIZED, null, requestUrl);
      global.__WATHIQA_LAST_API_ERROR__ = { method, url: requestUrl, message: error.message, status: error.status };
      dispatchApiEvent("wathiqa:api-error", global.__WATHIQA_LAST_API_ERROR__);
      throw error;
    }

    if (response.status === HTTP_STATUS.FORBIDDEN) {
      const error = createError(FORBIDDEN_MESSAGE, HTTP_STATUS.FORBIDDEN, null, requestUrl);
      global.__WATHIQA_LAST_API_ERROR__ = { method, url: requestUrl, message: error.message, status: error.status };
      dispatchApiEvent("wathiqa:api-error", global.__WATHIQA_LAST_API_ERROR__);
      throw error;
    }
    ...
    if (!response.ok) {
      ...
      const message = (payload && payload.message) || (payload && payload.error) || "حدث خطأ في الطلب";
    ...
    const result = await parseResponse(response, options.responseType ?? "json");
    global.__WATHIQA_LAST_API_ERROR__ = null;
    dispatchApiEvent("wathiqa:api-success", { method, url: requestUrl, result });
    return result;
  }

  function api(endpoint, options = {}) { return api.get(endpoint, options); }
  api.get = function (url, options = {}) { return request("GET", url, options); };
  api.post = function (url, data, options = {}) { return request("POST", url, { ...options, data }); };
  api.put = function (url, data, options = {}) { return request("PUT", url, { ...options, data }); };
  api.patch = function (url, data, options = {}) { return request("PATCH", url, { ...options, data }); };
  api.delete = function (url, options = {}) { return request("DELETE", url, options); };
  api.upload = function (url, formData, options = {}) { return request("POST", url, { ...options, body: formData, data: formData }); };
  api.download = function (url, options = {}) { return request("GET", url, { ...options, responseType: "blob" }); };
  api.request = request;
  Object.freeze(api);
  global.api = api;
})(window);
