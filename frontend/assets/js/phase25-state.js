(function (global) {
  ...
  function applyCaseValidation() {
    ["edit_case_title", "edit_court_case_number", "edit_case_type", "edit_court_name", "edit_court_chamber", "edit_opened_at", "hearing_date", "payment_amount", "payment_date", "expense_type", "expense_amount", "expense_date"].forEach((id) => {
      const element = document.getElementById(id); if (element) element.required = true;
    });
    ["edit_total_fees", "payment_amount", "expense_amount"].forEach((id) => {
      const element = document.getElementById(id); if (!element) return;
      element.type = "number"; element.min = "0"; element.step = "0.01"; element.inputMode = "decimal";
    });
  }
  ...
  function applyPasswordValidation() {
    ["newPassword", "confirmPassword"].forEach((id) => {
      const element = document.getElementById(id); if (!element) return;
      element.required = true; element.minLength = 8; element.autocomplete = "new-password";
    });
  }
  ...
  function init() {
    const pagesWithState = [isCaseProfile(), isClientProfile(), isDashboard(), isServiceProfile(), isServices(), isCalendar()];
    ...
    if (!pagesWithState.some(Boolean)) return;
