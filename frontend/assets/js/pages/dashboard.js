...
(function (global) {
  ...
  function updateClock() {
    state.elements.time.textContent = new Date().toLocaleTimeString("ar-EG", {
      hour: "2-digit",
      minute: "2-digit",
    });

    state.elements.date.textContent = new Date().toLocaleDateString("ar-EG", {
      weekday: "long",
      day: "numeric",
      month: "long",
      year: "numeric",
  ...
  function renderHero(data) {
    ...
    state.elements.officeName.textContent =
      office.officeName || "مكتب المحاماة";

    state.elements.summary.textContent =
      `لديك اليوم ${summary.hearingsToday ?? 0} جلسات، ` +
      `${summary.urgentTasks ?? 0} مهام عاجلة، ` +
      `${summary.notifications ?? 0} إشعارات جديدة.`;
  ...
  function renderNotifications(data) {
    ...
    notifications.forEach((item) => {
      ...
      element.className =
        "flex items-start gap-4 p-5 hover:bg-gray-50 transition";
