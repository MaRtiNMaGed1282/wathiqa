...
(function (global) {
  ...
  function highlightActiveLink() {
    ...
    const currentPage = normalizePage(global.location.pathname).replace(".html", "");
  ...
  async function fetchSidebar() {
    ...
    try {
      const response = await fetch(CONFIG.sidebarPath, { signal: controller.signal });
  ...
  function updateOfficeInformation(office) {
    ...
    state.elements.officeRole.textContent = office.ownerName ?? office.role ?? "";
  }
  ...
  async function loadSidebar() {
    ...
    state.loadingPromise = (async () => {
      ...
      if (state.elements.sidebar && state.container?.contains(state.elements.sidebar)) {
        applyRoleVisibility();
        highlightActiveLink();
        return;
      }
