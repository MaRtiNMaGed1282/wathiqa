...
(function (global) {
  ...
  function clearElements() {
    Object.keys(state.elements).forEach((key) => { state.elements[key] = null; });
  }
  ...
  async function loadNavbar() {
    ...
    try { return await state.loadingPromise; }
  ...
  async function initializeGlobalSearch() {
    ...
    if (!global.GlobalSearch) await loadGlobalSearchScript();
  ...
  function loadGlobalSearchScript() {
    const existingScript = document.querySelector(`script[src="${CONFIG.globalSearchScript}"]`);
    if (existingScript) return waitForGlobalSearch(existingScript);
    return new Promise((resolve, reject) => {
      const script = document.createElement("script");
      script.src = CONFIG.globalSearchScript;
      script.defer = true;
      script.onload = resolve;
      script.onerror = () => reject(new Error("Unable to load globalSearch.js."));
  ...
  function waitForGlobalSearch(script) {
    return new Promise((resolve, reject) => {
      if (global.GlobalSearch) { resolve(); return; }
      script.addEventListener("load", resolve, { once: true });
      script.addEventListener("error", () => reject(new Error("Unable to load globalSearch.js.")), { once: true });
    });
  }
  ...
  global.refreshNotificationBadge = refreshNotificationBadge;
  ...
  global.Navbar = Object.freeze({
    load: loadNavbar,
    async reload() { destroy(); return loadNavbar(); },
    destroy,
