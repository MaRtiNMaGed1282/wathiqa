(function (global) {
  ...
  function toast(message, success = true) {
    if (global.Toastify) global.Toastify({ text: message, duration: 3200, gravity: 'top', position: 'left', close: true }).showToast();
  ...
  function ensureModal() {
    ...
    modal.addEventListener('click', (event) => {
      ...
      if (event.target.id === 'savePermissions') savePermissions();
    });
    return modal;
  }
  ...
  async function openPermissions(userId, userName) {
    ensureModal(); selectedUserId = Number(userId);
    document.getElementById('permissionsUserLabel').textContent = userName || `المستخدم #${userId}`;
    const body = document.getElementById('permissionsBody');
    body.innerHTML = '<div class="py-12 text-center text-gray-500">جاري تحميل الصلاحيات...</div>';
  ...
  async function savePermissions() {
    ...
    const button = document.getElementById('savePermissions'); button.disabled = true;
    try {
      const permissions = {};
      ...
      toast('تم حفظ الصلاحيات بنجاح'); closeModal();
    } catch (error) { toast(error?.message || 'تعذر حفظ الصلاحيات', false); }
  ...
  async function editUser(user) {
    ...
    const result = await global.Swal.fire({
      ...
      html: `<div class="text-right space-y-3"><input id="editUserName" class="swal2-input" placeholder="الاسم الكامل" value="${escapeHtml(user.full_name)}"><input id="editUserUsername" class="swal2-input" placeholder="اسم المستخدم" value="${escapeHtml(user.username || String(user.email || '').split('@')[0])}"><select id="editUserRole" class="swal2-select"><option value="admin">مدير النظام</option><option value="lawyer">محامي</option><option value="assistant">مساعد</option></select></div>`,
      didOpen: () => { document.getElementById('editUserRole').value = user.role || 'assistant'; },
    ...
    try { await global.api.put(`/permissions/users/${encodeURIComponent(user.id)}/profile`, result.value); toast('تم تحديث المستخدم بنجاح'); global.location.reload(); }
    catch (error) { toast(error?.message || 'تعذر تحديث المستخدم', false); }
  }
  ...
  function enhanceUsersTable() {
    ...
    body.querySelectorAll('tr').forEach((row) => {
      ...
      const statusButton = row.querySelector('[data-action="status"]');
      ...
      const user = { id: Number(id), full_name: name, email: row.querySelector('td .text-sm.text-gray-500')?.textContent?.trim() || '', role: row.querySelector('td:nth-child(2)')?.textContent?.trim() || '' };
      ...
      container.insertBefore(editButton, deleteButton); container.insertBefore(permissionsButton, deleteButton);
  ...
  function init() {
    ...
    enhanceUsersTable(); global.__WATHIQA_USER_PERMISSIONS_LOADED__ = true;
  }
