...
const { loadConfig, saveConfig, normalizeConfig, getBackendUrl, DEFAULT_PORT } = require("./deployment-config");
...
// The setup utility and the main Wathiqa application must share one deployment.json.
...
async function testServer(serverUrl) {
  ...
  let info = null;
  try {
    info = await requestJson(`${normalized.serverUrl}/api/system/info`, 5000);
  } catch (_) {}
...
function registerIpc() {
  ipcMain.handle("setup:get-state", () => {
    ...
    return {
      config,
      network,
      firewall: getFirewallDiagnostics(config.port || DEFAULT_PORT),
    };
  });

  ipcMain.handle("setup:save-standalone", (_event, payload = {}) => {
    return saveConfig({
      mode: "standalone",
      host: "127.0.0.1",
      serverUrl: null,
      serverIdentity: payload.serverIdentity || null,
  ...
  ipcMain.handle("setup:save-server", (_event, payload = {}) => {
    const config = saveConfig({
      ...
      host: "0.0.0.0",
      serverUrl: null,
      serverIdentity: payload.serverIdentity || os.hostname(),
      port: payload.port || DEFAULT_PORT,
    });
    ...
    return {
      ...
      firewall: getFirewallDiagnostics(config.port),
    };
  });
  ...
  ipcMain.handle("setup:save-client", (_event, payload = {}) => {
    const test = payload.serverUrl ? null : "serverUrl is required";
  ...
  ipcMain.handle("setup:open-url", (_event, url) => shell.openExternal(String(url)));
}
