...
function startClientConnectionMonitor() { const config = getRuntimeConfig(); if (config.mode !== "client" || !config.serverUrl) return; connectionMonitorStop = startConnectionMonitor({ serverUrl: config.serverUrl, intervalMs: 15000, onStateChange: ({ state }) => { if (state === "offline") showConnectionNotification("Wathiqa server unavailable", "The office server cannot be reached. Wathiqa will keep checking and will reconnect automatically."); else if (state === "online") showConnectionNotification("Wathiqa server connected", "Connection to the office server has been restored."); } }); }
