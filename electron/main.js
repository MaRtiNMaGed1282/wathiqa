...
function sendHeartbeat() {
  ...
  let parsed;
  try {
    parsed = new URL(`${getRuntimeBackendUrl()}/api/system/devices/heartbeat`);
  } catch (_) {
    return;
  }
  ...
  const request = http.request(parsed, {
    ...
    headers: {
      ...
      "X-Wathiqa-Device-Token": identity.deviceToken,
    },
  }, (response) => {
    response.resume();
    if (response.statusCode < 200 || response.statusCode >= 300) {
      console.warn(`Wathiqa device heartbeat rejected: HTTP ${response.statusCode}`);
    }
  });
...
function showConnectionNotification(title, body) {
  ...
  try {
    ...
    console.warn("Unable to show Wathiqa notification:", error.message);
  }
}
...
function startClientConnectionMonitor() {
  ...
  connectionMonitorStop = startConnectionMonitor({
    ...
    onStateChange: ({ state, error }) => {
      if (state === "offline") {
        console.warn("Wathiqa office server is offline:", error?.message || "unknown error");
        showConnectionNotification(
          "Wathiqa server unavailable",
          "The office server cannot be reached. Wathiqa will keep checking and will reconnect automatically."
        );
      } else if (state === "online") {
        console.log("Wathiqa office server is reachable again.");
        showConnectionNotification(
          "Wathiqa server connected",
          "Connection to the office server has been restored."
        );
      }
    },
  });
...
function configureNavigation(win) {
  win.webContents.setWindowOpenHandler(({ url }) => (
    !url || url === "about:blank" ? { action: "allow" } : { action: "deny" }
  ));
...
function createStartupErrorWindow(error, options = {}) {
  ...
  const title = options.title || (isClient ? "Wathiqa server unavailable" : "Wathiqa could not start");
  const message = options.message || (isClient
  ...
  startupWindow.loadURL("data:text/html;charset=utf-8," + encodeURIComponent(`
    <!doctype html>
    <html lang="en">
      <head><meta charset="utf-8"><title>Wathiqa</title></head>
      <body style="font-family:sans-serif;padding:40px;line-height:1.6">
        <h2>${title}</h2>
        <p>${message}</p>
        <p>Please verify that the Wathiqa server is running and both computers are connected to the same office network.</p>
        <p><strong>Recovery:</strong> Wathiqa will automatically retry the server connection.</p>
      </body>
    </html>
  `));
...
async function completeClientRecovery() {
  try {
    await waitForServer(5000);
    const license = await checkLicense();
    if (!license?.valid) {
      createStartupErrorWindow(new Error("The office Wathiqa license is invalid or expired."), {
        title: "Wathiqa license unavailable",
        message: "The client is connected to the office server, but the server reports that the office license is not valid. License activation must be performed on the main server computer.",
      });
      return false;
    }
...
function startClientRecovery() {
  ...
  stopRecoveryTimer();
  const retry = async () => {
    if (await completeClientRecovery()) return;
...
async function startApplication() {
  ...
  try {
    ...
    if (deploymentConfig.mode === "client") {
      throw new Error(`Unable to validate the office license through the server: ${error.message}`);
    }
    license = { valid: false };
  }
  ...
  if (deploymentConfig.mode === "client") {
    if (!license?.valid) {
      throw new Error("The office Wathiqa license is invalid or expired. License activation must be performed on the main server computer.");
    }
    createWindow(LOGIN_PAGE);
    startHeartbeat();
    startClientConnectionMonitor();
