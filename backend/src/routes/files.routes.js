...
const { resolveStoredFile } = require("../services/fileStorage.service");
...
function sendStoredUpload(res, filename) {
  ...
  const absolutePath = resolveStoredFile(safeFilename);
...
router.get("/by-name/:filename", auth, (req, res) => {
  ...
  db.get(`SELECT file_path FROM case_files WHERE file_name = ?`, [filename], (err, file) => {
