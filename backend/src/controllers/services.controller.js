...
function validateServicePayload(body, { allowFees = true } = {}) {
  ...
  if (!validateDate(body.start_date) || !validateDate(body.due_date)) {
  ...
  if (allowFees && !isValidMoney(body.total_fees ?? 0)) {
    return "Invalid total_fees";
  }

  if (body.completed_date && !validateDate(body.completed_date)) {
    return "Invalid completed_date";
  }

  if (body.service_status === COMPLETED_STATUS && !body.completed_date) {
    return "completed_date is required when service is completed";
  }

  if (body.service_status !== COMPLETED_STATUS && body.completed_date) {
    return "completed_date must be empty unless service is completed";
  }
...
function getNextServiceNumber(callback) {
  db.get(
    `
    SELECT COALESCE(MAX(CAST(SUBSTR(service_number, 5) AS INTEGER)), 0) AS max_number
    FROM legal_services
...
exports.searchServices = (req, res) => {
  ...
  db.all(
    `${serviceSelect()}
     WHERE legal_services.service_number LIKE ?
        OR legal_services.service_title LIKE ?
        OR legal_services.service_type LIKE ?
        OR legal_services.assigned_to LIKE ?
...
exports.filterServices = (req, res) => {
  ...
  if (status) {
    query += " AND legal_services.service_status = ?";
    params.push(status);
  }
  if (type) {
    query += " AND legal_services.service_type = ?";
    params.push(type);
  }
  if (priority) {
    query += " AND legal_services.priority_level = ?";
    params.push(priority);
  }
  if (assignedTo) {
    query += " AND legal_services.assigned_to = ?";
    params.push(assignedTo);
  }
  if (fromDate) {
    query += " AND legal_services.start_date >= ?";
    params.push(fromDate);
  }
  if (toDate) {
    query += " AND legal_services.due_date <= ?";
    params.push(toDate);
  }
...
exports.createService = (req, res) => {
  ...
  const {
    client_id, service_type, service_title, description, service_status,
    total_fees, start_date, due_date, completed_date, assigned_to, notes, priority_level,
  } = req.body;
  ...
  db.get("SELECT id FROM clients WHERE id = ?", [client_id], (clientErr, client) => {
    ...
    getNextServiceNumber((numberErr, serviceNumber) => {
      ...
      db.run(
        ...
        `,
        [
          client_id, serviceNumber, service_type, service_title, description || null,
          service_status, isAssistant ? 0 : Number(total_fees || 0), start_date, due_date,
          completed_date || null, assigned_to || null, notes || null, priority_level || null,
        ],
        function (err) {
...
exports.updateService = (req, res) => {
  ...
  const {
    client_id, service_title, service_type, assigned_to, start_date, due_date,
  ...
  db.get("SELECT id FROM clients WHERE id = ?", [client_id], (clientErr, client) => {
    ...
    const params = [
      service_title, service_type, assigned_to || null, start_date, due_date,
      service_status, priority_level || null, description || null,
    ...
    db.run(
      ...
      SET service_title = ?, service_type = ?, assigned_to = ?, start_date = ?,
          due_date = ?, service_status = ?, priority_level = ?, description = ?,
          completed_date = ?, notes = ?, updated_at = CURRENT_TIMESTAMP${feeClause}
      WHERE service_id = ?
      `,
      params,
      function (err) {
...
exports.getClientServices = (req, res) => {
  db.all(
    `
    SELECT service_id, service_number, service_title, service_type, service_status,
           total_fees, start_date, due_date, completed_date, assigned_to, priority_level
    FROM legal_services
    WHERE client_id = ?
